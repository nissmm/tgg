...
def get_sender_display(record: dict) -> str:
    """Возвращает строку с никнеймом/ID отправителя анкеты."""
    requester_user = record.get("requester_username")
    req_id = record.get("requester_id")
    
    if requester_user and requester_user not in ("—", "(без username)"):
        if req_id:
            return f"{requester_user} ({req_id})"
        return str(requester_user)
    if req_id:
        return f"ID {req_id}"
    
    hr_user = record.get("hr_username")
    if hr_user and hr_user != "Самозапись" and hr_user != "—":
        return str(hr_user)
        
    recorded_by = record.get("recorded_by")
    if recorded_by and recorded_by != "Самозапись" and recorded_by != "—":
        return str(recorded_by)
        
    if is_self_apply_record(record):
        return "Кандидат (самозапись)"
...
def format_channel_notification(record: dict) -> str:
    ...
    sender = get_sender_display(record)
    return "\n".join(
        [
            "🆕 <b>Новая запись HR</b>",
            "",
            f"Отправил(а): {html.escape(sender)}",
...
def format_channel_approved_notification(record: dict, reviewer_username: str) -> str:
    """Уведомление в рабочий чат об одобрении заявки."""
    sender = get_sender_display(record)
    return "\n".join(
        [
            f"✅ <b>Заявка #{record['id']} одобрена</b>",
            f"Кандидат: {html.escape(str(record['candidate_info']))}",
            f"Телефон: <code>{html.escape(str(record['phone']))}</code>",
            f"Юз: {html.escape(str(record.get('username') or '—'))}",
            f"Время собеса: {html.escape(str(record.get('interview_datetime', '—')))}",
...
def format_channel_postponed_notification(record: dict, reviewer_username: str) -> str:
    """Уведомление в рабочий чат об отложении заявки."""
    sender = get_sender_display(record)
    return "\n".join(
        [
            f"⏳ <b>Заявка #{record['id']} отложена на рассмотрение</b>",
            f"Кандидат: {html.escape(str(record['candidate_info']))}",
            f"Отправил(а): {html.escape(sender)}",
            f"Обработал(а): {html.escape(reviewer_username)}",
        ]
    )


def format_channel_rejected_notification(record: dict, reviewer_username: str) -> str:
    """Уведомление в рабочий чат об отклонении заявки."""
    sender = get_sender_display(record)
    return "\n".join(
        [
            f"❌ <b>Заявка #{record['id']} отклонена</b>",
            f"Кандидат: {html.escape(str(record['candidate_info']))}",
            f"Телефон: <code>{html.escape(str(record['phone']))}</code>",
            f"Юз: {html.escape(str(record.get('username') or '—'))}",
            f"Отправил(а): {html.escape(sender)}",
